...
class SwordFishStrategy(Strategy):
    """
        ...
        **Идея**:
        ...
        – в трёх разных строках оно может находиться
          только в трёх одних и тех же колонках,
        то образуется структура Swordfish.

        Аналогично:
        – в трёх разных колонках число N может находиться
          только в трёх одних и тех же строках.

        **Следствие**:
        Число N не может находиться ни в одной другой ячейке
        этих колонок (или строк соответственно) и исключается.
    """

    def apply(self, field: RectangleField) -> tuple[RectangleField, bool]:
        exclude_cell_coordinates = set()

        # === Swordfish по строкам ===
        for base_row_order, base_row in enumerate(LineBuilder.build_all_row_lines(field), 1):
            ...
            for value in possible_values:
                ...
                if not (2 <= len(base_candidates) <= 3):
                    continue
                ...
                if len(rows_with_same_columns) != 3:
                    continue
        ...
        for base_column_order, base_column in enumerate(LineBuilder.build_all_column_lines(field), 1):
            ...
            for value in possible_values:
                ...
                if len(columns_with_same_rows) != 3:
                    continue
