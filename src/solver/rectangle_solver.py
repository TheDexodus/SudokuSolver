...
class RectangleSolver(Solver):
    ...
    def _find_solo_possible_value(self, field: Field) -> None:
    ...
    def _find_solo_possible_value_for_grid(self, grid: GridCellRepository) -> None:
    ...
    def _fill_uno_possible_value(self, field: Field) -> None:
        for x in range(1, field.get_size()[0] + 1):
            for y in range(1, field.get_size()[1] + 1):
    ...
    def _and_possible_cells_by_row(self, field: Field) -> None:
        for y in range(1, field.get_size()[1] + 1):
            for grid_y in range(1, field.get_size()[1] + 1):
    ...
    def _and_possible_cells_by_col(self, field: Field) -> None:
        for x in range(1, field.get_size()[0] + 1):
            for grid_x in range(1, field.get_size()[0] + 1):
    ...
    def _and_possible_cells_by_grid(self, field: Field) -> None:
        for y in range(1, field.get_size()[1] + 1):
            for x in range(1, field.get_size()[0] + 1):
                grid = field.get_grid(Coordinate(x, y))
